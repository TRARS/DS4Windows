...
namespace CustomMacroFactory.MVVM.Helpers
{
    internal sealed partial class Manager
    {
        ...
        readonly ImageColorPickerVM _imageColorPickerVM;

        public Manager(ImageColorPickerVM imageColorPickerVMFactory)
        {
            _imageColorPickerVM = imageColorPickerVMFactory;
        }
        ...
        {
            var type = typeof(ImageColorPickerVM).Name;

            //
            if (this.Begin(type)) { return; }

            // 生成VM
            var imageColorPickerVM = _imageColorPickerVM;
